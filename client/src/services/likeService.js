...
export const getLikeId = async (crystalId, _ownerId) => {
    ...
    const whereClause = `where=crystalId%3D%22${crystalId}%22%20AND%20_ownerId%3D%22${_ownerId}%22`;
...
export const getIsLikedByCurrentUser = async (likeId, crystalId, loggedUserId) => {
    const result = await request.get(`${BASE_URL}/${likeId}`)

    if (result._ownerId === loggedUserId && result.crystalId === crystalId) {
        return true
    } else {
        return false;
    }
...
// export const dislike = async (crystalId, isLiked) => {
//     const newDislike = await request.put(BASE_URL, {
//         crystalId,
//         isLiked,
//     });
